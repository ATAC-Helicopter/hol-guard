...
@final
@dataclass(frozen=True, slots=True)
class VersionedPackageSubcommandMatcher:
    """Match a launcher executable, a package token, and a fixed subcommand suffix.

    The package token may carry an npm-style `@<version-or-tag>` suffix (as in
    `npx pkg@latest` or `npm exec pkg@1.2.3`), which an exact-token subcommand
    comparison cannot express. Reuses the same known-option stripping and flag
    semantics as `ExecutableMatcher` so behavior is identical once the package
    token is normalized; the only new logic is that normalization step.
    """

    executables: frozenset[str]
    package: str
    leading_subcommands: tuple[str, ...] = ()
    subcommands: tuple[str, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        normalized_package = self.package.strip().lower()
        normalized_leading_subcommands = tuple(
            value.strip().lower() for value in self.leading_subcommands if value.strip()
        )
        normalized_subcommands = tuple(value.strip().lower() for value in self.subcommands if value.strip())
        normalized_required_flags = frozenset(value.strip().lower() for value in self.required_flags if value.strip())
        normalized_interspersed_options = frozenset(
        ...
        if not normalized_executables or not normalized_package:
            raise ValueError("VersionedPackageSubcommandMatcher requires executables and a package name")
        if "@" in normalized_package:
            raise ValueError("VersionedPackageSubcommandMatcher package name cannot itself contain '@'")
        object.__setattr__(self, "executables", normalized_executables)
        object.__setattr__(self, "package", normalized_package)
        object.__setattr__(self, "leading_subcommands", normalized_leading_subcommands)
        object.__setattr__(self, "subcommands", normalized_subcommands)
        object.__setattr__(self, "required_flags", normalized_required_flags)
        object.__setattr__(self, "interspersed_options_with_values", normalized_interspersed_options)
        object.__setattr__(self, "interspersed_flags", normalized_interspersed_flags)
    ...
    def match(self, command: CanonicalCommand) -> tuple[MatcherEvidence, ...]:
        evidence: list[MatcherEvidence] = []
        prefix = (*self.leading_subcommands, self.package, *self.subcommands)
        package_prefix = f"{self.package}@"
        for index, segment in enumerate(command.segments):
            if not _segment_matches_executable(segment, self.executables):
                continue
            lowered_arguments = tuple(argument.lower() for argument in segment.arguments)
            filtered = _without_options(
                lowered_arguments,
                self.interspersed_options_with_values,
                self.interspersed_flags,
            )
            ...
            if normalized[: len(prefix)] != prefix:
                continue
            flag_arguments = normalized[len(prefix) :]
            semantics = argument_semantics(
                flag_arguments,
                options_with_values=self.interspersed_options_with_values,
            ...
            evidence.append(
                MatcherEvidence(
                    segment_index=index,
                    executable=segment.executable,
                    detail="Matched a package launcher command with an optional version or tag specifier.",
                )
            )
        return tuple(evidence)
